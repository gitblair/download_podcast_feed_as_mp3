...
$result = $conn->query("SELECT name, description, imageUrl, rssUrl FROM taddypodcasts");
$podcasts = [];
...
if (isset($_GET['search'])) {
    ...
    $stmt = $conn->prepare("SELECT name, description, imageUrl, rssUrl FROM taddypodcasts WHERE name = ?");
...
// echo "<pre>";
// var_dump($podcast_info, $rssUrl);
// echo "</pre>";



                $stmt = $conn->prepare("INSERT INTO taddypodcasts (name, description, imageUrl, rssUrl) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("ssss", $podcast_info['title'], $podcast_info['description'], $podcast_info['imageUrl'], $rssUrl);
                $stmt->execute();
