...
// prob with listen notes is they want $ for rss feed retrieval
...
// Create connection
// use a config to supply credentials
...
// Search for new podcasts using Listen Notes API
    ...
    if (!empty($_GET['search_query'])) {
        // use a config to supply credentials
        $api_key = $listen_notes_api_key;
        //$api_key = '5b7b2d22eaa84ca4a1396e2a0148ff8b';
        $search_query = urlencode($_GET['search_query']);
        $sort_by_date = $_GET['sort_by_date'] ?? 1;
        $type = $_GET['type'] ?? 'podcast';
        $offset = $_GET['offset'] ?? 0;
        $len_min = $_GET['len_min'] ?? 0;
        $len_max = $_GET['len_max'] ?? 0;
        $genre_ids = $_GET['genre_ids'] ?? '';
        $published_before = $_GET['published_before'] ?? 0;
        $published_after = $_GET['published_after'] ?? 0;
        $only_in = $_GET['only_in'] ?? 'title';
        $language = $_GET['language'] ?? 'English';
        $safe_mode = $_GET['safe_mode'] ?? 0;
        $unique_podcasts = $_GET['unique_podcasts'] ?? 1;
        $interviews_only = $_GET['interviews_only'] ?? 0;
        $sponsored_only = $_GET['sponsored_only'] ?? 0;
        $page_size = $_GET['page_size'] ?? 10;

        $api_url = "https://listen-api.listennotes.com/api/v2/search?q=$search_query&sort_by_date=$sort_by_date&type=$type&offset=$offset&len_min=$len_min&len_max=$len_max&genre_ids=$genre_ids&published_before=$published_before&published_after=$published_after&only_in=$only_in&language=$language&safe_mode=$safe_mode&unique_podcasts=$unique_podcasts&interviews_only=$interviews_only&sponsored_only=$sponsored_only&page_size=$page_size";
